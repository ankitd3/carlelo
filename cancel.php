...
$can=$_POST["cancel"]; //Booking TIME(Timestamp)
date_default_timezone_set('Asia/Kolkata');
$current_datetime = date('Y-m-d H:i:s');

$sql1 = "SELECT * FROM booking WHERE Bid='$can'";
$result1 = $conn->query($sql1);
if ($result1->num_rows > 0) {
    // output data of each row
    while($row = $result1->fetch_assoc()) {
        $print = "<h4 style=color:black>Booking ID:".$row["Bid"]."</h4>CAR id: " . $row["car_id"]. " <br>PICK:      " . $row["pickup"]. " <br>DROP:     " . $row["drop_car"]."<br>Amount Paid: Rs.".$row["amount"]. "/-<br>Booking TIME: ". $row["booikingtime"]."<br>";
        $pick=$row["pickup"];
        $amt=$row["amount"];
    }
    if($pick>$current_datetime)
    {
      $sql = "DELETE FROM booking WHERE (Bid='$can')";

      if ($conn->query($sql) === TRUE) {
          $printcancel="Successfully CANCELLED the above booking, AMOUNT RS.".$amt."/- refunded to the same payment method";
      }else {
          $printcancel = "Error deleting record: " . $conn->error;
      }
    }
    else{
      $printcancel="As it is not a future booking, it cannot be cancelled, sorry!";
    }
}
else{
  $print="<h2>Enter valid booking ID</h2>";
  $printcancel="";
}
...
<a href="mybooking.php"><h2 class="w3-text-blue" style="float: left"> Hi <?php echo $_SESSION['userData']['first_name']; ?></h2></a><h2 style="float:right; color: white;"><a href="login/logout.php">LOGOUT</a></h2>
  ...
  <div class="w3-container">
    <h2>The booking selected:</h2>
    <div align="center" class="w3-text-blue w3-light-grey"><?php echo $print;?></div>
    <h2><?php echo $printcancel;?></h2>
