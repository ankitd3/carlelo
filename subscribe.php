...
$sql = "UPDATE users SET subscribe_email='$to' WHERE email='$troll'";

if ($conn->query($sql) === TRUE) {
    $var = "Thank you for subscribing with email : ".$to.", PLease check your inbox for instant offers";
} else {
    $var = "Error updating record: " . $conn->error;
}

$subject = "Hi!";
 $body = "Hi,\n\nGlad you wished to receive offers and newsletters from CARLELO.com\n\nExclusive discount of Rs 399 on your next booking if you show us this mail :)";
 if (mail($to, $subject, $body)) {
   $var1 = "<p>Successfully SUBSCRIBED!</p>";
  } else {
   $var1 = "<p>Email delivery failed…</p>";
  }
  $conn->close();
?>
<!DOCTYPE html>
...
<a href="mybooking.php"><h2 class="w3-text-blue" style="float: left"> Hi <?php echo $_SESSION['userData']['first_name']; ?></h2></a><h2 style="float:right; color: white;"><a href="login/logout.php">LOGOUT</a></h2>
  ...
  <h2 class="w3-text-blue w3-light-grey"><strong><?php echo $var;?></strong></h2>
  <hr>
  </div>
  <hr>
